...
void MyMesh::GenerateDiamond(uint a_uSides, float a_fHeight, float a_fWidth, vector3 a_v3Color) 
{
	...
	std::vector<vector3> otherBaseVertex;
	...
	GLfloat angleChange = (2 * PI) / a_uSides;
	...
	for (int i = 0; i < a_uSides; i++)
	{
		...
		vector3 tempBase = vector3(cos(startAngle) * a_fWidth, 0, sin(startAngle) * a_fWidth);
		vector3 otherBase = vector3(cos(startAngle) * a_fWidth, 0, sin(startAngle) * a_fWidth);
		baseVertex.push_back(tempBase);
		otherBaseVertex.push_back(otherBase);
		startAngle += angleChange;
	}

	//Loopoing through based on the the number of subdivisions and drawing the triangles required to draw the diamond
	for (int i = 0; i < a_uSides; i++)
	{
		AddTri(baseVertex[i], vector3(0, -a_fHeight / 2, 0), baseVertex[(i + 1) % a_uSides]);
		AddTri(baseVertex[i], baseVertex[(i + 1) % a_uSides], vector3(0, a_fHeight / 2, 0));

		AddTri(otherBaseVertex[i], vector3(0, a_fHeight / 2, 0), otherBaseVertex[(i + 1) % a_uSides]);
		AddTri(otherBaseVertex[i], otherBaseVertex[(i + 1) % a_uSides], vector3(0, -a_fHeight / 2, 0));
	}
